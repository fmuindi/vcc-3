...
const BASE_STYLE = `<style>
  *{box-sizing:border-box}
  ...
  .reel-scroller{position:relative;padding:0 56px}
  .reel-track{display:flex;gap:18px;overflow-x:auto;scroll-behavior:smooth;-webkit-overflow-scrolling:touch;scrollbar-width:none;-ms-overflow-style:none;padding:10px 2px 16px}
  .reel-track::-webkit-scrollbar{display:none;height:0}
  .reel-item{position:relative;display:block;flex:none;width:190px;aspect-ratio:9/16;border-radius:18px;overflow:hidden;background:#100E0D;box-shadow:0 14px 34px rgba(16,14,13,.2);border:0;padding:0;margin:0;font:inherit;color:inherit;cursor:pointer;transition:transform .3s ease,box-shadow .3s ease}
  ...
  .reel-item::after{content:'';position:absolute;inset:0;background:linear-gradient(0deg,rgba(16,14,13,.6),rgba(16,14,13,0) 45%)}
  .reel-play-btn{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:54px;height:54px;border-radius:50%;background:#E01B2E;display:grid;place-items:center;color:#fff;font-size:18px;box-shadow:0 8px 22px rgba(224,27,46,.55);z-index:2;transition:transform .2s ease}
  .reel-item:hover .reel-play-btn{transform:translate(-50%,-50%) scale(1.08)}
  .reel-scroll-btn{position:absolute;top:50%;transform:translateY(-50%);width:52px;height:52px;border-radius:50%;background:#fff;color:#100E0D;border:1.5px solid rgba(16,14,13,.12);display:grid;place-items:center;font-size:17px;cursor:pointer;box-shadow:0 10px 26px rgba(16,14,13,.18);z-index:5;transition:background .18s ease,color .18s ease,transform .18s ease,opacity .18s ease}
  .reel-scroll-btn:hover{background:#E01B2E;color:#fff;border-color:#E01B2E;transform:translateY(-50%) scale(1.06)}
  .reel-scroll-prev{left:0}
  .reel-scroll-next{right:0}
  .reel-scroll-btn:disabled{opacity:.3;cursor:default;pointer-events:none}
  @media (max-width:640px){.reel-scroller{padding:0 44px}.reel-item{width:150px}.reel-scroll-btn{width:40px;height:40px;font-size:14px}}
  .social-follow-grid{display:flex;flex-wrap:wrap;justify-content:center;gap:30px}
  .social-follow-btn{display:flex;flex-direction:column;align-items:center;gap:14px;width:112px;color:#100E0D}
  .social-follow-icon{width:82px;height:82px;border-radius:50%;display:grid;place-items:center;color:#fff;font-size:32px;box-shadow:0 16px 34px rgba(16,14,13,.2);transition:transform .25s cubic-bezier(.2,.8,.2,1),box-shadow .25s ease}
  .social-follow-btn:hover .social-follow-icon{transform:translateY(-8px) scale(1.07);box-shadow:0 22px 46px rgba(16,14,13,.3)}
  .social-follow-label{font-size:13px;font-weight:700;letter-spacing:.03em;color:rgba(16,14,13,.68)}
  @media (max-width:640px){.social-follow-grid{gap:16px}.social-follow-btn{width:78px}.social-follow-icon{width:60px;height:60px;font-size:23px}.social-follow-label{font-size:11px}}
  .event-card{position:relative;display:block;flex:none;width:230px;border-radius:18px;overflow:hidden;background:#fff;box-shadow:0 14px 34px rgba(16,14,13,.16);text-decoration:none;color:inherit;transition:transform .3s ease,box-shadow .3s ease}
  .event-card:hover{transform:translateY(-6px);box-shadow:0 22px 46px rgba(16,14,13,.26)}
  .event-card-img{position:relative;height:150px;background:#100E0D}
  .event-card-img img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover}
  .event-card-date{position:absolute;top:12px;left:12px;background:#fff;border-radius:10px;padding:7px 12px;text-align:center;box-shadow:0 6px 16px rgba(16,14,13,.22)}
  .event-card-date .mon{display:block;font-size:10px;font-weight:800;letter-spacing:.06em;color:#E01B2E;text-transform:uppercase}
  .event-card-date .day{display:block;font-size:16px;font-weight:800;color:#100E0D;font-family:'Bricolage Grotesque',sans-serif;line-height:1.1}
  .event-card-body{padding:16px 16px 18px}
  .event-card-title{font-family:'Bricolage Grotesque',sans-serif;font-weight:800;font-size:15px;color:#100E0D;margin-bottom:4px}
  .event-card-detail{font-size:12.5px;color:rgba(16,14,13,.55);margin-bottom:10px;min-height:16px}
  .event-card-meta{display:flex;flex-direction:column;gap:4px;font-size:12px;color:rgba(16,14,13,.5)}
  @media (max-width:640px){.event-card{width:190px}.event-card-img{height:126px}}
  .events-search-row{display:flex;gap:12px;margin-bottom:20px;flex-wrap:wrap}
  .events-search-input{flex:1;min-width:180px;padding:14px 18px;border-radius:12px;border:1.5px solid rgba(16,14,13,.15);font-size:14px;font-family:inherit;background:#fff}
  .events-view-toggle{display:flex;border-radius:12px;overflow:hidden;border:1.5px solid rgba(16,14,13,.15);flex:none}
  .events-view-btn{padding:0 18px;background:#fff;border:0;font-size:12.5px;font-weight:700;letter-spacing:.03em;color:rgba(16,14,13,.55);cursor:pointer;display:flex;align-items:center;gap:8px}
  .events-view-btn.is-active{background:#100E0D;color:#fff}
  .events-filter-row{display:flex;flex-wrap:wrap;gap:10px;margin-bottom:30px}
  .events-filter-pill{padding:9px 18px;border-radius:999px;border:1.5px solid rgba(16,14,13,.15);background:#fff;font-size:12px;font-weight:700;letter-spacing:.03em;color:rgba(16,14,13,.6);cursor:pointer;transition:background .15s ease,color .15s ease,border-color .15s ease}
  .events-filter-pill.is-active{background:#100E0D;color:#fff;border-color:#100E0D}
  .events-list-row{display:flex;align-items:center;gap:20px;padding:18px 0;border-bottom:1px solid rgba(16,14,13,.08)}
  .events-list-date{flex:none;width:60px;text-align:center}
  .events-list-date .dow{display:block;font-size:11px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:rgba(16,14,13,.45)}
  .events-list-date .dom{display:block;font-size:15px;font-weight:800;color:#100E0D;font-family:'Bricolage Grotesque',sans-serif}
  .events-list-time{flex:none;width:84px;font-size:13px;color:rgba(16,14,13,.55)}
  .events-list-info{flex:1;min-width:0}
  .events-list-title{font-weight:700;font-size:15px;color:#100E0D;margin-bottom:2px}
  .events-list-loc{font-size:13px;color:rgba(16,14,13,.5)}
  .events-list-link{flex:none;font-size:12px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:#E01B2E;white-space:nowrap}
  .events-empty{padding:44px 0;text-align:center;color:rgba(16,14,13,.5);font-size:14px}
  @media (max-width:640px){.events-list-time{display:none}.events-list-link{display:none}}
  .key-date-item{display:flex;gap:14px;padding:14px 0;border-bottom:1px solid rgba(16,14,13,.08)}
  .key-date-icon{flex:none;width:36px;height:36px;border-radius:10px;background:rgba(224,27,46,.08);display:grid;place-items:center;color:#E01B2E;font-size:14px}
  .key-date-label{font-weight:700;font-size:14px;color:#100E0D;margin-bottom:2px}
  .key-date-value{font-size:13px;color:rgba(16,14,13,.55)}
  .reel-lightbox-overlay{position:fixed;inset:0;z-index:200;background:rgba(16,14,13,.86);display:none;align-items:center;justify-content:center;padding:60px 24px;opacity:0;transition:opacity .25s ease;overflow-y:auto}
  .reel-lightbox-overlay.is-open{display:flex}
  .reel-lightbox-overlay.is-visible{opacity:1}
  .reel-lightbox-card{position:relative;width:100%;max-width:340px;margin:auto}
  .reel-lightbox-frame{position:relative;width:100%;aspect-ratio:9/16;border-radius:18px;overflow:hidden;background:#100E0D;box-shadow:0 30px 80px rgba(0,0,0,.55)}
  .reel-lightbox-close{position:absolute;top:-46px;right:0;width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,.14);color:#fff;border:none;display:grid;place-items:center;cursor:pointer;font-size:20px;line-height:1;transition:background .15s ease}
  .reel-lightbox-close:hover{background:rgba(255,255,255,.26)}
  ...
  .va-progress{margin-bottom:20px;background:#fff;border-radius:14px;box-shadow:0 10px 30px rgba(16,14,13,.1);padding:14px 20px;box-sizing:border-box}
  .va-progress.is-stuck{position:fixed;z-index:20;box-shadow:0 14px 34px rgba(16,14,13,.18)}
  .va-progress-label{display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;font-size:12px;font-weight:700;letter-spacing:.07em;text-transform:uppercase;color:rgba(16,14,13,.55)}
  .va-progress-percent{color:#E01B2E;font-weight:800;font-variant-numeric:tabular-nums}
  .va-progress-bar{position:relative;height:9px;border-radius:999px;background:rgba(16,14,13,.08);overflow:hidden}
  .va-progress-fill{height:100%;width:0%;border-radius:999px;background:linear-gradient(90deg,#E01B2E,#F02338);transition:width .35s ease}
...
class Component extends DCLogic {
  componentDidMount() {
    ...
    this.stickyProgress();
    this.videoReelLightbox();
    this.scrollerControls();
    this.eventsFinder();
  }
  scrollerControls() {
    // Generic arrow-button horizontal scroller: works for any
    // [data-scroller] wrapper containing [data-scroll-track] +
    // [data-scroll-prev]/[data-scroll-next] buttons (currently the Get
    // Involved video reel and the Events & Calendar "Coming Up" carousel).
    // Native scrollbar is hidden via CSS (scrollbar-width:none /
    // ::-webkit-scrollbar{display:none}) — scrolling is driven entirely by
    // these arrow buttons instead.
    ...
    scrollers.forEach((root) => {
      ...
      const prevBtn = root.querySelector('[data-scroll-prev]');
      const nextBtn = root.querySelector('[data-scroll-next]');
  ...
  eventsFinder() {
    // No-op on pages without the Events & Calendar "Find an Event" widget.
    const root = document.getElementById('events-finder');
    if (!root) return;
    const searchInput = document.getElementById('events-search-input');
    const pills = Array.from(root.querySelectorAll('.events-filter-pill'));
    const rows = Array.from(root.querySelectorAll('.events-list-row'));
    const emptyState = document.getElementById('events-empty-state');
    const listPanel = document.getElementById('events-list-panel');
    const monthPanel = document.getElementById('events-month-placeholder');
    const listBtn = document.getElementById('events-view-list');
    const monthBtn = document.getElementById('events-view-month');
    // Same hidden-attribute-stripping bug as everywhere else on this site
    // -- assert the initial state by hand instead of trusting the server-
    // rendered hidden attribute to survive.
    ...
    const applyFilters = () => {
      ...
      rows.forEach((row) => {
        ...
        const show = (activeCategory === 'All' || cat === activeCategory) && (!q || text.includes(q));
        row.hidden = !show;
    ...
    // Month view is a placeholder for now -- there's only ever one real
    // category (Chapel) of events to plot on a calendar grid today, so
    // toggling it swaps in a friendly message instead of a full month grid.
    if (listBtn && monthBtn && listPanel && monthPanel) {
      listBtn.addEventListener('click', () => {
        listBtn.classList.add('is-active'); monthBtn.classList.remove('is-active');
        listPanel.hidden = false; monthPanel.hidden = true;
      });
      monthBtn.addEventListener('click', () => {
        monthBtn.classList.add('is-active'); listBtn.classList.remove('is-active');
        monthPanel.hidden = false; listPanel.hidden = true;
      });
    }
  }
  videoReelLightbox() {
    // No-op on pages without a video reel (currently only Get Involved).
    // Reel tiles are static thumbnails, not live embedded iframes — a
    // cross-origin YouTube iframe's own internal play-button click can't be
    // observed from the parent page, so a live iframe per tile would make
    // "open a lightbox on play" and "only one video at a time" impossible
    // to implement. Using one shared lightbox with a single iframe instead
    // means only one video can ever be loaded/playing at once, by
    // construction — no separate "stop the others" bookkeeping needed.
    const overlay = document.getElementById('reel-lightbox');
    if (!overlay) return;
    const iframe = document.getElementById('reel-lightbox-iframe');
    const closeBtn = document.getElementById('reel-lightbox-close');
    const triggers = Array.from(document.querySelectorAll('.reel-item[data-video-id]'));
    ...
    const close = () => {
      ...
      setTimeout(() => {
        ...
        iframe.src = ''; // stop playback/audio once the lightbox is closed
      }, 250);
    };
    triggers.forEach((el) => {
      el.addEventListener('click', () => open(el.getAttribute('data-video-id')));
    });
  ...
  stickyProgress() {
    // No-op on pages without a .va-progress bar. CSS position:sticky does
    // not work here because #page-top (an ancestor of every .va-progress
    // bar) has overflow-x:hidden — verified empirically that this silently
    // breaks position:sticky in this runtime even though only the x-axis
    // is affected — so this pins the bar with position:fixed by hand
    // instead, with a spacer left behind so the page doesn't jump.
    const bars = Array.from(document.querySelectorAll('.va-progress'));
    if (!bars.length) return;
    const headerOffset = () => (window.innerWidth <= 1200 ? 64 : 92);
    bars.forEach((bar) => {
      const spacer = document.createElement('div');
      spacer.className = 'va-progress-spacer';
      spacer.style.height = '0px';
      bar.parentElement.insertBefore(spacer, bar);
      let stuck = false;
      let originalTop = bar.getBoundingClientRect().top + window.scrollY;
      const syncRect = () => {
      ...
      const onScroll = () => {
        ...
        const shouldStick = window.scrollY >= originalTop - off;
        if (shouldStick && !stuck) stick();
        else if (!shouldStick && stuck) unstick();
        else if (stuck) { bar.style.top = off + 'px'; syncRect(); }
      };
      window.addEventListener('scroll', onScroll, { passive: true });
      window.addEventListener('resize', () => { if (stuck) { syncRect(); bar.style.top = headerOffset() + 'px'; } else { originalTop = bar.getBoundingClientRect().top + window.scrollY; } });
      onScroll();
    });
  }
  scholarshipApplicationForm() {
    ...
    const progressFill = document.getElementById('scholarship-progress-fill');
    const progressPercent = document.getElementById('scholarship-progress-percent');
    ...
    // Percentage progress bar: share of required fields (unique per
    // va-field, so radio/checkbox groups only count once) currently valid.
    ...
    submitBtn.addEventListener('click', () => {
      ...
      setTimeout(() => {
        form.hidden = true;
        const progress = root.querySelector('.va-progress');
        if (progress) { progress.hidden = true; progress.classList.remove('is-stuck'); }
        const progressSpacer = root.querySelector('.va-progress-spacer');
        if (progressSpacer) progressSpacer.hidden = true;
        confirmation.hidden = false;
        confirmation.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 700);
    });
  ...
  applicationWizard() {
    ...
    // Same visibility walk as isVisible, but ignores the .va-page hidden
    // flag: progress is tracked across all 3 steps at once (not just the
    // current one), while still excluding fields hidden by a conditional
    // panel or a program-catalog toggle.
    const isVisibleForProgress = (el) => {
      let node = el;
      while (node && node !== root) {
        if (node.hidden && !node.classList.contains('va-page')) return false;
        node = node.parentElement;
      }
      return true;
    };
    ...
    const updateProgress = () => {
      ...
      Array.from(form.querySelectorAll('[data-required="true"]')).forEach((el) => {
        if (el.disabled) return;
        if (!isVisibleForProgress(el)) return;
        const key = el.closest('.va-field') || el;
        if (seen.has(key)) return;
        seen.add(key);
        total += 1;
    ...
    const addCollegeRow = () => {
      ...
      item.querySelector('.va-repeater-remove').addEventListener('click', () => { item.remove(); updateProgress(); });
