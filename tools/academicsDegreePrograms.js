...
// TODO: only one program is confirmed so far (given in the client brief as
// an explicit example). Replace/extend this list once the full degree
// catalog (name, one-sentence description, delivery format) is provided —
// do not invent additional degree names.
const DEGREE_PROGRAMS = [
  {
    name: 'Bachelor of Arts in Ministry',
    copy: 'Prepare for effective ministry through biblical education, leadership development, and practical experience.',
  },
];

const requestInfoLink = (program) => `admissions.html?program=${encodeURIComponent(program)}`;

const degreeCard = (p) => `      <div data-reveal="" style="background:#fff;border:1px solid rgba(16,14,13,.08);border-radius:16px;padding:32px 28px">
        ...
        <p style="margin:0 0 22px;font-size:14.5px;line-height:1.6;color:rgba(16,14,13,.65)">${p.copy}</p>
...
const ACADEMICS_DEGREE_PROGRAMS_BODY = `  <section id="degree-hero" style="position:relative;min-height:68vh;display:flex;align-items:flex-end;overflow:hidden;background:#100E0D;padding-top:70px">
  ...
  <section id="degree-programs" style="max-width:1320px;margin:0 auto;padding:100px 32px 40px">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,340px));justify-content:center;gap:18px">
${DEGREE_PROGRAMS.map(degreeCard).join('\n')}
    </div>
  </section>
